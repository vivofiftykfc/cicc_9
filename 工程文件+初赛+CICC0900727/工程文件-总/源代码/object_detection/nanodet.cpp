...
NanoDet::NanoDet(const char* param, const char* bin, bool useGPU)
{
    ...
    this->Net->opt.num_threads = 4; // 在加载模型前设置线程数
...
std::vector<BoxInfo> NanoDet::detect(cv::Mat image, float score_threshold, float nms_threshold)
{
    ...
    // ex.set_num_threads(4); // 已废弃，由 net.opt.num_threads 控制
    // ex.set_vulkan_compute(this->hasGPU); // 已废弃，由 net.opt.use_vulkan_compute 控制
    ex.input("data", input);
